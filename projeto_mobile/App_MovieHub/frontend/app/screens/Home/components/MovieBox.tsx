    ...
    return (
        ...
        >
            ...
            <View style={{ marginLeft: 12, flex: 1, justifyContent: "center" }}>
                <Text style={style.movieRowTitle} numberOfLines={2}>{item.title}</Text>

                <View style={{ flexDirection: "row", alignItems: "center", marginTop: 6 }}>
                    <Text style={style.textAverage}>{item.vote_average?.toFixed(1) ?? "N/A"}/10 IMDb</Text>
                </View>

                <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 8 }}>
                    {genreNames.map((name) => (
                        <View key={name} style={style.genrePill}>
                            <Text style={style.genresText}>{name.toUpperCase()}</Text>
                        </View>
                    ))}
                </View>

                {runtime ? (
                    <Text style={style.runtimeText}>🕐 {formatRuntime()}</Text>
                ) : null}
            </View>
        </TouchableOpacity>
    );
